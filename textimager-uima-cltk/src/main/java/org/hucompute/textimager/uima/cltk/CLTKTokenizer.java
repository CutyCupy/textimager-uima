...
public class CLTKTokenizer extends CLTKBase {
	...
	return new JSONObject().put("lang", aJCas.getDocumentLanguage()).put("text", aJCas.getDocumentText());
    }
	...
	for (Object t : tokens) {
		...
		while (end < docText.length()
			&& (Character.isAlphabetic(docText.charAt(end)) || Character.isDigit(docText.charAt(end)))) {
		    end++;
		}
	    } else {
