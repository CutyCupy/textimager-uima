...
@TypeCapability(inputs = { "org.dkpro.core.api.segmentation.type.Token",
	"org.dkpro.core.api.segmentation.type.Sentence", "org.dkpro.core.api.lexmorph.type.pos.POS",
	"org.dkpro.core.api.lexmorph.type.pos.Lemma", "org.dkpro.core.api.lexmorph.type.morph.Morpheme" }, outputs = {
		"org.dkpro.core.api.coref.type.CoreferenceChain", "org.dkpro.core.api.coref.type.CoreferenceLink",
...
public class OpenerProjectConstituentCoref extends JCasAnnotator_ImplBase {
	...
	modelProvider = new CasConfigurableProviderBase<File>() {
		...
		setDefault(LOCATION,
			"classpath:org/hucompute/textimager/uima/OpenerProject/lib/" + "constituent-${variant}.model");
	...
	if (jRubyLocation != null)
	    pathToJruby = jRubyLocation;

	// command for the Process
	List<String> cmd = new ArrayList<String>();
	cmd.add("/bin/sh");
	cmd.add("-c");
	cmd.add("export PATH=/usr/bin:$PATH && cat" + " \"" + KAF_LOCATION + "\"" + "| " + pathToJruby
		+ "jruby -S constituent-parser | " + pathToJruby + "jruby -S coreference");
