...
@TypeCapability(outputs = { "org.dkpro.core.api.segmentation.type.Token",
...
//			KAF_STRING = KAF_STRING.replace("![CDATA[", "").replace(".]]", "");
	...
	try {
		...
		int End = kafSentence.get(kafSentence.size() - 1).getOffset()
			+ kafSentence.get(kafSentence.size() - 1).getLength();
