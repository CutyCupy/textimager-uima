...
public class VerbsDisambiguation extends JCasAnnotator_ImplBase {
	...
	modelProvider = new ModelProviderBase<JFastText>() {
		...
		setDefault(LOCATION, "classpath:/${package}/lib/verbsdisambiguation-${language}-${variant}.properties");
	    ...
	    @Override
	    protected JFastText produceResource(URL aUrl) throws IOException {
	...
	try {
		...
		lines = IOUtils.readLines(getClass().getClassLoader()
			.getResourceAsStream("org/hucompute/textimager/disambiguation/verbs/lib/verbLemmaIds"));
	    else
    ...
    public static HashSet<String> getReflexivVerbs(GermaNet gnet)
	...
	for (LexUnit lex : gnet.getLexUnits(WordCategory.verben)) {
	    if (lex.getFrames().stream().anyMatch(
		    x -> x.toString().toLowerCase().contains("ar") || x.toString().toLowerCase().contains("dr"))
		    && gnet.getLexUnits(lex.getOrthForm(), WordCategory.verben).size() == 2) {
		if (containsR.contains(lex.getOrthForm()))
		    doubled.add(lex.getOrthForm());

		if (lex.getFrames().stream().allMatch(
			x -> x.toString().toLowerCase().contains("ar") || x.toString().toLowerCase().contains("dr")))
	...
	for (Sentence sentence : JCasUtil.select(aJCas, Sentence.class)) {
		...
		if (token.getPos().getClass() == POS_VERB.class) {
			...
			for (POS_PRON pr : JCasUtil.selectCovered(POS_PRON.class, sentence)) {
			    if (pr.getPosValue().equals("PRF") && lemma.contains(JCasUtil
				    .selectCovered(Dependency.class, pr).get(0).getGovernor().getLemma().getValue())) {
				containsPRF = true;
			    }
			}

			for (LexUnit lexUnit : gnet.getLexUnits(lemma, WordCategory.verben)) {
			    if (containsPRF && lexUnit.getFrames().stream()
				    .anyMatch(x -> x.toString().toLowerCase().contains("ar")
					    || x.toString().toLowerCase().contains("dr"))) {
				WordSense sense = new WordSense(aJCas, token.getBegin(), token.getEnd());
			...
			for (String string : sentence.getCoveredText().split(",|;|:| und ")) {
			    if (sentence.getCoveredText().indexOf(string) <= (token.getBegin() - sentence.getBegin())
				    && sentence.getCoveredText().indexOf(string) + string.length() >= token.getEnd()
					    - sentence.getBegin()) {
				toAnalize = (string);
				break;
			    }
			}
			toAnalize = toAnalize.replace("\"", " \"").replace(",", " ,").replace(".", " . ")
				.replace(":", " :").replace(";", " ;").replace("?", " ?").replace("!", " !")
				.replace("(", " (").replace(")", " )").replace("-", " -").replaceAll(" -(\\w)", " - $1")
				.trim();
