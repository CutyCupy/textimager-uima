...
public class TeiReader extends ResourceCollectionReaderBase {
	...
	/**
	 * Use the xml:id attribute on the TEI elements as document ID. Mind that many
	 * TEI files may not have this attribute on all TEI elements and you may end up
	 * with no document ID at all. Also mind that the IDs should be unique.
	 */
	...
	/**
	 * When not using the XML ID, use only the filename instead of the whole URL as
	 * ID. Mind that the filenames should be unique in this case.
	 */
	 ...
	 */
	// REC: This does not seem to work. Maybe because SAXWriter does not generate
	// this event?
	...
	/**
	 * Use this part-of-speech tag set to use to resolve the tag set mapping instead
	 * of using the tag set defined as part of the model meta data. This can be
	 * useful if a custom model is specified which does not have such meta data, or
	 * it can be used in readers.
	 */
	...
	public void initialize(UimaContext aContext) throws ResourceInitializationException {
		...
		if (readPOS && !readToken) {
			throw new ResourceInitializationException(
					new IllegalArgumentException("Setting readPOS to 'true' requires writeToken to be 'true' too."));
		}
		...
		posMappingProvider = MappingProviderFactory.createPosMappingProvider(null, mappingPosLocation, posTagset,
				getLanguage());
	}
	...
	public void getNext(CAS aCAS) throws IOException, CollectionException {
		...
		try {
			...
			boolean first = true;
			// while(DocumentMetaData.get(aCAS).getDocumentTitle().startsWith("Liste von")
			// ||DocumentMetaData.get(aCAS).getDocumentTitle().startsWith("Diskussion") ||
			// first){
			first = false;
			// Create handler
	...
	protected abstract static class Handler extends DefaultHandler {
		private JCas jcas;
	...
	private class DivDataHolder {
		...
		public DivDataHolder(int divStart, String divTyp, String divId, String divSection, String divUser,
				String divTimestamp) {
...
//		private String divTimestamp = null;
		...
		@Override
		public void startElement(String aUri, String aLocalName, String aName, Attributes aAttributes)
				throws SAXException {
			// System.out.printf("%b START %s %n", captureText, aLocalName);
			if (!inTextElement && TeiConstants.TAG_TEI_DOC.equals(aName)) {
				if (useXmlId) {
					documentId = aAttributes.getValue("xml:id");
				} else if (useFilenameId) {
					documentId = FilenameUtils.getName(currentResource.getPath()) + "#" + currentTeiElementNumber;
				} else {
					documentId = currentResource.getPath() + "#" + currentTeiElementNumber;
				}
			} else if (!inTextElement && TeiConstants.TAG_TITLE.equals(aName)) {
				captureText = true;
				titleStart = getBuffer().length();
			} else if (TeiConstants.TAG_TEXT.equals(aName)) {
				captureText = true;
				inTextElement = true;
				...
				String divTyp = aAttributes.getValue(TeiConstants.ATTR_TYPE);
				...
				headTyp = aAttributes.getValue(TeiConstants.ATTR_TYPE);
			...
			} else if (inTextElement && (TeiConstants.TAG_SUNIT.equals(aName)
					|| (utterancesAsSentences && TeiConstants.TAG_U.equals(aName)))) {
				sentenceStart = getBuffer().length();
			} else if (inTextElement && TeiConstants.TAG_PARAGRAPH.equals(aName)) {
				paragraphStart = getBuffer().length();
			} else if (readNamedEntity && inTextElement
					&& TeiConstants.TAG_RS.equals(aName)) {
				NamedEntity ne = new NamedEntity(getJCas());
				ne.setBegin(getBuffer().length());
				ne.setValue(aAttributes.getValue(TeiConstants.ATTR_TYPE));
				namedEntities.push(ne);
			} else if (readConstituent && inTextElement
					&& TeiConstants.TAG_PHRASE.equals(aName)) {
				...
				constituent.setConstituentType(
						aAttributes.getValue(TeiConstants.ATTR_TYPE));
				constituent.setSyntacticFunction(
						aAttributes.getValue(TeiConstants.ATTR_FUNCTION));
				constituents.push(new ConstituentWrapper(constituent));
			} else if (inTextElement && (TeiConstants.TAG_WORD.equals(aName)
					|| TeiConstants.TAG_CHARACTER.equals(aName)
					|| TeiConstants.TAG_MULTIWORD.equals(aName))) {
				tokenStart = getBuffer().length();

				if (StringUtils
						.isNotEmpty(aAttributes.getValue(TeiConstants.ATTR_POS))) {
					posTag = aAttributes.getValue(TeiConstants.ATTR_POS);
				} else {
					posTag = aAttributes.getValue(TeiConstants.ATTR_TYPE);
				}
		...
		public void endElement(String aUri, String aLocalName, String aName) throws SAXException {
			if (TeiConstants.TAG_TEI_DOC.equals(aName)) {
				inLiteratur = false;
			}
			if (!inTextElement && TeiConstants.TAG_TITLE.equals(aName)) {
				...
				if (headStart > -1 && (getBuffer().substring(headStart).trim().contains("Literatur")
						|| getBuffer().substring(headStart).trim().contains("Siehe auch")) && !takeLiteratur) {
				...
				wrapper.constituent.setChildren(FSCollectionFactory.createFSArray(getJCas(), wrapper.children));
				...
				if (constituents.peek().constituent instanceof ROOT) {
					...
					rootWrapper.constituent
							.setChildren(FSCollectionFactory.createFSArray(getJCas(), rootWrapper.children));
				...
				if (isNotBlank(getBuffer().substring(tokenStart, getBuffer().length()))) {
					...
					if (posTag != null && readPOS) {
						...
						POS pos = (POS) getJCas().getCas().createAnnotation(posTagType, token.getBegin(),
								token.getEnd());
					...
					if (casee != null || number != null || gender != null || person != null || tense != null
							|| mood != null || degree != null || voice != null) {
						// GrammaticalCategory cat = new
						// GrammaticalCategory(getJCas(),token.getBegin(),token.getEnd());
						...
						MorphologicalFeatures morph = new MorphologicalFeatures(getJCas(), token.getBegin(),
								token.getEnd());
						morph.setCase(casee);
	...
	private void construct(JCas cas, HashMap<Integer, Word> wordsInSentence) {
		for (Word word : wordsInSentence.values()) {
			if (word.depHead == 0) {
				de.tudarmstadt.ukp.dkpro.core.api.syntax.type.dependency.ROOT dependency = new de.tudarmstadt.ukp.dkpro.core.api.syntax.type.dependency.ROOT(
						cas, word.token.getBegin(), word.token.getEnd());
