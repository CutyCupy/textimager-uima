...
public class JSONReader extends JCasAnnotator_ImplBase {
	...
	// JCas cas = parseInputText(currentText.getString("text"),
	// currentText.getString("name"), offset);
	...
	// ");
	// DocElement element = new DocElement(aJCas, offset,
	// offset+currentText.getString("text").length()+1);
	...
	// ");
	// DocElement element = new DocElement(aJCas, offset,
	// offset+cas.getDocumentText().length()+1);
	// element.setName(currentText.getString("name"));
	//
	// element.addToIndexes();
	//
	// List<Sentence> sentences = IteratorUtils.toList(select(cas,
	// Sentence.class).iterator());
	...
	// }
	// List<Token> tokens = IteratorUtils.toList(select(cas,
	...
	for (int i = 0; i < sentences.length(); i++) {
		...
		if (text.length() > 0 && !currentElement.getString("word_form").equals("."))
		    text += " ";
		Token token = new Token(cas, text.length(),
			text.length() + currentElement.getString("word_form").length());
