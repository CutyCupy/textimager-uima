...
/**
 * Reads the contents of a given URL and strips the HTML. Returns the textual
 * contents. Also recognizes headings and paragraphs.
 */
@ResourceMetaData(name = "HTML Reader")
//@DocumentationResource("${docbase}/format-reference.html#format-${command}")
//@MimeTypeCapability({MimeTypes.APPLICATION_XHTML, MimeTypes.TEXT_HTML})
@TypeCapability(outputs = { "org.dkpro.core.api.metadata.type.DocumentMetaData",
	"org.dkpro.core.api.segmentation.type.Heading", "org.dkpro.core.api.segmentation.type.Paragraph",
	"org.dkpro.core.api.metadata.type.MetaDataStringField" })
public class EnhancedHtmlReader extends JCasResourceCollectionReader_ImplBase {
    /**
    ...
    /**
     * Name of configuration parameter that contains the character encoding used by
     * the input files.
     */
    public static final String PARAM_SOURCE_ENCODING = ComponentParameters.PARAM_SOURCE_ENCODING;
    @ConfigurationParameter(name = PARAM_SOURCE_ENCODING, mandatory = true, defaultValue = "UTF-8") // ComponentParameters.DEFAULT_ENCODING)
	...
	NodeTraversor traversor = new NodeTraversor(new NodeVisitor() {
		...
		if (node instanceof TextNode) {
		    ...
		    if (builder.length() > 0 && (element.isBlock() || element.nodeName().equals("br"))
		...
		if (node instanceof TextNode) {
			...
			if (type == MetaDataStringField.type) {
			    ...
			    trim(builder, span);
			    Div div = (Div) cas.createAnnotation(aJCas.getCasType(type), span[0], span[1]);
