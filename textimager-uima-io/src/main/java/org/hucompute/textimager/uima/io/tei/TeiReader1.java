...
public class TeiReader1 {
    ...
    /**
     * (character) contains a significant punctuation mark as identified by the
     * CLAWS tagger.
     */
    ...
    /**
     * contains a single text of any kind, whether unitary or composite, for example
     * a poem or drama, a collection of essays, a novel, a dictionary, or a corpus
     * sample.
     */
     ...
     * @author Richard Eckart de Castilho
     */
    public abstract static class Handler extends DefaultHandler {
	private JCas jcas;
	...
	public void endElement(String aUri, String aLocalName, String aName) throws SAXException {
		...
		if (isNotBlank(getBuffer().substring(tokenStart, getBuffer().length()))) {
			...
			MorphologicalFeatures cat = new MorphologicalFeatures(getJCas(), token.getBegin(),
				token.getEnd());
