...
public class TeiWriter extends JCasFileWriter_ImplBase {
    /**
     * Specify the suffix of output files. Default value <code>.xml</code>. If the
     * suffix is not needed, provide an empty string as value.
     */
    ...
    /**
     * A token matching this pattern is rendered as a TEI "c" element instead of a
     * "w" element.
     */
    ...
    /**
     * Write constituent annotations to the CAS. Disabled by default because it
     * requires type priorities to be set up (Constituents must have a higher prio
     * than Tokens).
     */
    ...
    /**
     * Write named entity annotations to the CAS. Overlapping named entities are not
     * supported.
     */
	...
	// attributes.add(xmlef.createAttribute("depType", c.getDependencyType()));
	// }
	// attributes.add(xmlef.createAttribute("depIndex",
	// Integer.toString(depIndex.get(c.getDependent()))));
	// attributes.add(xmlef.createAttribute("parent",
	// Integer.toString(depIndex.get(c.getGovernor()))));
	//
	// // if (c.getGovernor() != null) {
	// // attributes.add(xmlef.createAttribute(ATTR_FUNCTION,
	// c.getSyntacticFunction()));
	// // }
	// }
