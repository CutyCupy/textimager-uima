 ...
 */
@TypeCapability(inputs = { "org.dkpro.core.api.metadata.type.DocumentMetaData" })
public class HUComputeNER extends JCasAnnotator_ImplBase {
    ...
    /**
     * Variant of a model the model. Used to address a specific model if here are
     * multiple models for one language.
     */
	...
	mappingProvider.setDefaultVariantsLocation("org/hucompute/textimager/uima/ner/lib/ner-default-variants.map");
	mappingProvider.setDefault(MappingProvider.LOCATION,
		"classpath:/org/hucompute/textimager/uima/ner/lib/ner-${language}-${variant}.map");
	...
	try {
	    if (!JCasUtil.select(aJCas, WikiDataHyponym.class).isEmpty()
		    && !JCasUtil.select(aJCas, Token.class).isEmpty()) {
		...
		Map<Token, Collection<WikiDataHyponym>> indexHyponym = JCasUtil.indexCovering(aJCas, Token.class,
			WikiDataHyponym.class);
		Map<Token, Collection<Timex3>> indexTimex = JCasUtil.indexCovering(aJCas, Token.class, Timex3.class);
		HashMap<Token, Collection<NamedEntity>> indexEntitiy = new HashMap<>(
			JCasUtil.indexCovering(aJCas, Token.class, NamedEntity.class));
		Map<WikiDataHyponym, Collection<WikipediaLink>> indexWikidataWikipedia = JCasUtil.indexCovered(aJCas,
			WikiDataHyponym.class, WikipediaLink.class);
		...
		for (Sentence sentence : JCasUtil.select(aJCas, Sentence.class)) {
			...
			if (indexEntitiy.containsKey(token) && !indexEntitiy.get(token).isEmpty()) {
				...
				switch (value) {
				case "PER":
				    Person_HumanBeing human = new Person_HumanBeing(aJCas, ent.getBegin(),
					    ent.getEnd());
				...
				case "ORG":
				    ...
				    break;
				case "LOC":
				    Location_Place location_Place = new Location_Place(aJCas, ent.getBegin(),
					    ent.getEnd());
			...
			if (!indexHyponym.get(token).isEmpty()) {
				...
				boolean isInstance = indexWikidataWikipedia.get(lowestDepth).iterator().next()
					.getIsInstance();
				Type type = mappingProvider
					.getTagType(lowestDepth.getId() + (isInstance ? "" : "_abstract"));
				Annotation ne = (Annotation) aJCas.getCas().createAnnotation(type,
					lowestDepth.getBegin(), lowestDepth.getEnd());
				if (ne instanceof NamedEntity) {
//									((NamedEntity)ne).setValue(classMap.get(lowestDepth.getId()));
				    ((NamedEntity) ne).setValue("http://www.wikidata.org/entity/"
...
//									((AbstractNamedEntity)ne).setValue(classMap.get(lowestDepth.getId())+"_abstract");
				    ((AbstractNamedEntity) ne).setValue("http://www.wikidata.org/entity/"
					    + indexWikidataWikipedia.get(lowestDepth).iterator().next().getWikiData());
				}
			...
			if ((token.getCoveredText().endsWith("heit") || token.getCoveredText().endsWith("heiten")
				|| token.getCoveredText().endsWith("keit") || token.getCoveredText().endsWith("keiten"))
				&& Character.isUpperCase(token.getCoveredText().charAt(0))
			...
			if (token.getLemma().getValue().endsWith("ismus")
				&& Character.isUpperCase(token.getLemma().getValue().charAt(0))
				&& JCasUtil.selectCovering(NamedEntity.class, token).isEmpty()) {
			...
			if ((float) countAnnos / countTokens > 0.3 && sentence.getCoveredText().endsWith(".")
				&& Character.isUpperCase(sentence.getCoveredText().charAt(0))
		...
		if (constraint) {
			...
			ArrayList<Token> tokens = new ArrayList<Token>(
				JCasUtil.selectCovered(Token.class, currSentence));
			...
			for (int i = 0; i < tokens.size(); i++) {
				...
				Annotation nean = (Annotation) aJCas.getCas().createAnnotation(type, ne.getBegin(),
					ne.getEnd());
				if (ne instanceof NamedEntity)
				    ne.setValue(phrasesClasses.get(ne.getCoveredText()));
			...
			for (int i = 0; i < tokens.size(); i++) {
				...
				AbstractNamedEntity ne = new AbstractNamedEntity(aJCas, beginToken.getBegin(),
					endToken.getEnd());
				Type type = mappingProvider
					.getTagType(phrasesClasses_abstract.get(ne.getCoveredText()));
				Annotation nean = (Annotation) aJCas.getCas().createAnnotation(type, ne.getBegin(),
					ne.getEnd());
				if (ne instanceof AbstractNamedEntity)
				    ((AbstractNamedEntity) ne)
					    .setValue(phrasesClasses_abstract.get(ne.getCoveredText()));
