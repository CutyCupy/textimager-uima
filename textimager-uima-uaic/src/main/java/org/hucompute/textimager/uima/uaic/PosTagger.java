...
public class PosTagger extends JCasAnnotator_ImplBase {
	...
	try {
	    UaicHybridPOStagger tagger = new UaicHybridPOStagger(ResourceStreamProvider.getModelFile(),
		    DictionaryProvider.getMorphologicalDictionary(), ResourceStreamProvider.getTagsetFile(),
		    ResourceStreamProvider.getReductionFile());
	    ...
	    // Keep track of the current position relative to the complete text by adding
	    // the length of every
	    // sentence to the total length. This corresponds the cursor movement over the
	    // text
		...
		for (int i = 0; i < sentence.getTokenCount(); i++) {
		    ...
		    // The processing of the whole text simultaneously could cause massive
		    // performance issues
		    ...
		    if (postag.length() > 2) {
			// The PoS identifiers are the first 2 characters in case the whole PoS string
			// is bigger
			// Overwrite it
		    ...
		    POS posElement = (POS) cas.createAnnotation(posTag,
			    textRelativePos + uaicToken.getCharStartIndexInSentence(),
			    textRelativePos + uaicToken.getCharEndIndexInSentence());
