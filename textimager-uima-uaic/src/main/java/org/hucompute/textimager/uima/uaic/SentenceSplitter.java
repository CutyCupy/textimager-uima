...
/**
 * This class provides the sentence splitting functionality for the romanian
 * language
 *
 * @author Dinu Ganea
 */
...
public class SentenceSplitter extends JCasAnnotator_ImplBase {
	...
	ro.uaic.info.nlptools.tools.UaicTokenizer uaicTokenizer = new ro.uaic.info.nlptools.tools.UaicTokenizer(
		DictionaryProvider.getMorphologicalDictionary());
	...
	try {
		...
		// To calculate the sentence length, we just query the last token and get its
		// end position.
		// We also keep track of the begin position of the sentence by keeping the total
		// length
