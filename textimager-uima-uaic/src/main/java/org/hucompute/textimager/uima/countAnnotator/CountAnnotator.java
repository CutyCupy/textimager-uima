...
@TypeCapability(inputs = { "org.dkpro.core.api.segmentation.type.Token", }, outputs = {
...
public class CountAnnotator extends JCasAnnotator_ImplBase {
	...
	// Count lemmata and word form apparition first while storing/updating the
	// counter into the specific HashMap
	select(jCas, Token.class).forEach(new Consumer<Token>() {
		...
		// We don't need to iterate over lemmata, we can extract them direct from the
		// token
		lemmata.put(token.getLemma().getValue(), lemmata.getOrDefault(token.getLemma().getValue(), 0) + 1);
	    }
	});

	// Parse the HashMap for each saved word form and create a CountAnnotation
	// instance
