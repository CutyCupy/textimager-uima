 ...
 * @author Dinu Ganea
 */
@TypeCapability(outputs = { "org.dkpro.core.api.segmentation.type.Lemma" })
public class Lemmatizer extends JCasAnnotator_ImplBase {
	...
	try {
	    ...
	    // text
	    // Must be done because token positions are relative to the sentence, not to the
	    // entire text
		...
		for (int i = 0; i < sentence.getTokenCount(); i++) {
		    ...
		    // Lemma begin position is added to the sentence begin position in the text to
		    // get the idx relative
		    // to the actual text. Same for the end idx
		    Lemma lemma = new Lemma(jCas, textRelativePos + tokenUaic.getCharStartIndexInSentence(),
		    ...
		    Set<UaicMorphologicalAnnotation> annotations = DictionaryProvider.getMorphologicalDictionary()
			    .get(tokenUaic.getWordForm());
		    if (annotations != null) {
