...
public class MediawikiWriter extends JCasConsumer_ImplBase {
	...
	try {
	    writer = new PrintWriter(
		    new OutputStreamWriter(new FileOutputStream(outputDumpFile), Charset.forName("UTF-8")));
    ...
    // This process is to get all information from TextImager-Client about the
    // document, paragraphs, sentences and words
	...
	for (Paragraph paragraph : JCasUtil.select(jCas, Paragraph.class)) {
		...
		if (!paragraphCats.isEmpty()) {
		    Collections.sort(paragraphCats, (r1,
			    r2) -> ((r1.getScore() > r2.getScore()) ? -1 : ((r1.getScore() < r2.getScore()) ? 1 : 0)));
		    CategoryCoveredTagged cat = paragraphCats.get(0);
		    catBuffer.append("[[").append(categoryPrefix).append("DDC")
			    .append(cat.getValue().replaceAll("__label_ddc__", "")).append("]]\n");
		...
		for (Token token : JCasUtil.selectCovered(Token.class, sentence)) {
		    ...
		    textBuffer.append("{{#word: ").append(token.getCoveredText()).append(" |lemma:")
			    .append(token.getLemma().getValue()).append(",pos:").append(token.getPos().getPosValue());
		    try {
...

		{
			...
			Collections.sort(sentenceCats, (r1, r2) -> ((r1.getScore() > r2.getScore()) ? -1
				: ((r1.getScore() < r2.getScore()) ? 1 : 0)));
			CategoryCoveredTagged cat = sentenceCats.get(0);
			textBuffer.append(" | DDC:").append(cat.getValue().replaceAll("__label_ddc__", "")).append("_")
				.append(MediawikiDDCHelper
					.getDDCClassName(cat.getValue().replaceAll("__label_ddc__", "")));
