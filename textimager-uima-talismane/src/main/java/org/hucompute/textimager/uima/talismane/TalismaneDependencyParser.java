...
public class TalismaneDependencyParser extends JCasAnnotator_ImplBase {
    ...
    /**
     * Use this language instead of the document language to resolve the model and
     * tag set mapping.
     */
    ...
    /**
     * Process anyway, even if the model relies on features that are not supported
     * by this component.
     * 
     * Default: {@code false}
     */
	...
	modelProvider = new CasConfigurableProviderBase<File>() {
		...
		setDefault(LOCATION,
			"classpath:org/hucompute/textimager/uima/talismane/lib/" + "dependency-${variant}.model");
	...
	for (de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Sentence jCasSentence : org.apache.uima.fit.util.JCasUtil
		.select(aJCas, de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Sentence.class)) {
		...
		Token token = tokenSequence.addToken(jCasToken.getBegin() - SentenceBegin,
			jCasToken.getEnd() - SentenceBegin);

		try {
		    posTagSequence.addPosTaggedToken(
			    new PosTaggedToken(token, new Decision(jCasToken.getPosValue()), session));
	...
	if ((pStart != pEnd)) {
	    parent = JCasUtil.selectSingleAt(aJCas, de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Token.class,
		    pStart, pEnd);
	} else {
	    parent = JCasUtil.selectSingleAt(aJCas, de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Token.class,
		    cStart, cEnd);
	...
	de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Token child = JCasUtil.selectSingleAt(aJCas,
		de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Token.class, cStart, cEnd);
